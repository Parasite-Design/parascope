...
def link_documents():
    allowed_collections = {
        ...
        "products",
        "customers",
    }  # Example: define allowed collections
    with MongoDbConnector() as mongo_connector:
        ...
        async def run_linker():
            for link in config.LINKING_CONFIG:
                print(
                    f"Linking documents from {link['remote_collection']} to {link['output_collection']}..."
                )
                output_collection = link.get("output_collection")
                if (
                    not output_collection
                    or output_collection not in allowed_collections
                ):
                    print(
                        f"Invalid or unauthorized output_collection: {output_collection}"
                    )
                    continue
                try:
                    pipeline = load_template(config.LINKING_TEMPLATE_PATH, link)  # pyright: ignore[reportArgumentType]
                    await (
                        mongo_connector.db[output_collection]
                        .aggregate(pipeline)
                        .to_list(length=None)

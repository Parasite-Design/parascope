...
async def migrate_collection(
    ...
    print(
        f"Starting migration of collection {collection_name} with instructions {instructions}"
    )
    ...
    await mongodb_connector.db[collection_name].create_index(
        identifier_field, unique=True
    ...
    runtime_variables = {
        ...
        "LAST_UPDATE": await mongodb_connector.db["updates"].find_one(
            sort=[("end_date", DESCENDING)]
        ),  # pyright: ignore[reportOptionalSubscript]
    }
    ...
    if not runtime_variables["LAST_UPDATE"]:
        ...
        runtime_variables["LAST_UPDATE"] = runtime_variables["LAST_UPDATE"][
            "start_date"
        ]
    ...
    if "SQL_PARAMS" in instructions.keys():
        collection = sql_connector.cursor.execute(
            query, (runtime_variables["LIMIT_DATE"], runtime_variables["LAST_UPDATE"])
        )  # pyright: ignore[reportOptionalMemberAccess]
    else:
        collection = sql_connector.cursor.execute(
            query, (runtime_variables["LAST_UPDATE"])
        )  # pyright: ignore[reportOptionalMemberAccess]
    ...
    for doc in docs:
        ...
        mongo_doc = sanitize_for_mongo(
            {path["MongoDb"]: doc[path["SQL"]] for path in paths if path["SQL"] in doc}
        )
            ...
            async def run_migrations():
                for collection_name, instructions in config.MIGRATION_CONFIG.items():
                    await migrate_collection(
                        collection_name, instructions, mongo_connector, sql_connector
                    )

    ...
    with MongoDbConnector() as mongo_connector:
        ...
        async def ensure_index_async(collection, index_spec, **options):
            if isinstance(index_spec, str):
                ...
                raise ValueError("index_spec must be a string or a list of (field, direction) tuples")
            else:
                idx_spec = index_spec
            existing_indexes = await collection.index_information()
            for index in existing_indexes.values():
                if set(tuple(pair) for pair in index['key']) == set(tuple(pair) for pair in idx_spec):
                    if 'unique' in options:
                        if index.get('unique', False) != options['unique']:
                            raise ValueError(f"Index on {idx_spec} already exists with different unique option")
                    return

          ...
          <el-table-column label="Objectives" min-width="350">
            <template #default="{ row }">
              <div class="objectives-list">
                <div
                  v-if="Object.keys(row.objectives).length === 0"
                  class="no-objectives"
                >
                ...
                <div v-else class="objectives-items">
                  ...
                  >
                    <div class="objective-header">
                      <strong>{{
                        getBrandDisplayName(String(brandName))
                      }}</strong>
                    </div>
                    <div class="objective-details">
                      <div
                        v-if="brandObj.salesPerCustomer"
                        class="objective-detail"
                      >
                        <span class="type-label">Sales per Customer:</span>
                        <span class="type-value">{{
                          formatValue(
                            brandObj.salesPerCustomer,
                            "salesPerCustomer",
                          )
                        }}</span>
                      </div>
                      <div
                        v-if="brandObj.activeCustomers"
                        class="objective-detail"
                      >
                        <span class="type-label">Active Customers:</span>
                        <span class="type-value">{{
                          formatValue(
                            brandObj.activeCustomers,
                            "activeCustomers",
                          )
                        }}</span>
                      </div>
                      <div
                        v-if="
                          ...
                          !brandObj.activeCustomers
                        "
                        class="no-objective-details"
                      >
                        <el-tag type="info" size="small"
                          >No objectives set for this brand</el-tag
                        >
                      </div>
                    </div>
                  </div>
    ...
    >
      ...
      >
        ...
        <el-form-item label="Brand" prop="brand_name">
          ...
          >
            <el-option label="All Brands" value="ALL" />
            <el-option
...
const formRules: FormRules = {
  ...
  value: [
    {
      required: true,
      message: "Please enter objective value",
      trigger: "blur",
    },
    {
      type: "number",
...
const submitForm = async () => {
  ...
  await objectiveFormRef.value.validate(async (valid) => {
    ...
    try {
      ...
      await api.post("/api/v1/settings/objective", payload);
      ...
      const errorMsg =
        error.response?.data?.detail || "Failed to set objective";

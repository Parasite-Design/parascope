    ...
    >
      ...
      >
        ...
        <el-form-item label="Display Name" prop="showed_brand_name">
          ...
          />
          <div class="form-hint">This is the name shown to users</div>
        </el-form-item>
      </el-form>
  ...
  await brandFormRef.value.validate(async (valid) => {
    ...
    try {
      if (formMode.value === "create") {
        await api.post("/api/v1/settings/brand", {
...
const deleteBrand = async (brand: Brand) => {
  try {
    ...
    await api.delete(`/api/v1/settings/brand/${brand.brand_name}`);

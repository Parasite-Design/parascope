      ...
      <el-row :gutter="20">
            ...
            <el-descriptions :column="1" border>
              <el-descriptions-item label="Current Period Total">
                <span
                  :class="{
                    'highlight-text':
                      customer.period1_total > customer.period2_total,
                  }"
                >
              ...
              <el-descriptions-item label="Next Visit">
                <el-tag
                  :type="getNextVisitStatus(customer.next_visit)"
                  v-if="customer.next_visit"
                >
              ...
              <el-descriptions-item label="Priority Score">
                <el-tag
                  :type="getScoreStatus(customer.score)"
                  v-if="customer.score !== undefined"
                >
                  {{ customer.score }}
                </el-tag>
                <span v-else>-</span>
        ...
        <el-col :span="16">
          ...
          <el-card class="notes-card" style="margin-top: 20px">
            ...
            <div
              class="notes-content"
              :class="{ 'empty-notes': !customer.note }"
            >
              {{
                customer.note ||
                'No notes available. Click "Add Notes" to add some.'
              }}
            </div>
          </el-card>
          ...
          <el-card class="visits-card" style="margin-top: 20px">
            ...
            <div class="visits-content">
              ...
              <div v-else class="no-data-placeholder">
                <el-empty
                  description="No visit history available"
                  :image-size="80"
                />
              </div>
            </div>
...
const hasCoordinates = computed(() => {
  return (
    ...
    props.customer.latitude !== "" &&
    props.customer.longitude !== "" &&
    !isNaN(parseFloat(props.customer.latitude)) &&
    !isNaN(parseFloat(props.customer.longitude))
  );
});
...
const hasVisits = computed(() => {
  return (
    detailedCustomer.value?.visits && detailedCustomer.value.visits.length > 0
  );
});
...
const editNotes = async () => {
  ...
  try {
    ...
    await api.put(`/api/v1/customers/${props.customer._id}`, updatedCustomer);
...
const toggleFavorite = async () => {
  ...
  try {
    ...
    ElMessage.success(
      props.customer.favorite ? "Added to favorites" : "Removed from favorites",
    );
    emit("refresh-data"); // Notify parent to refresh data
  } catch (error) {
...
const viewVisitDetails = (visit: Visit) => {
  ElMessageBox.alert(
    visit.notes || "No details available",
